...
public:
	...
	void computeAvoidanceVelocity(double target_vel, double target_rot, 
		...
		if (needEmergencyStop()) {
			...
			if (emergency_stop_counter > 10 && !in_recovery_mode) {
		...
		if (in_recovery_mode) {
			...
			if (laser_data != nullptr) {
				...
				// 如果找到空旷路径且已旋转足够角度
				if (clearance > 100.0 && fabs(angle_to_best) < 0.3) {
					path_found = true;
			...
			// 决定是否退出脱困模式
			if (path_found || recovery_rotation_frames > 60) {
				if (path_found) {
			...
			// 朝向最佳方向旋转
			adjusted_rot = (angle_to_best > 0) ? 0.5 : -0.5;
			
			if (recovery_rotation_frames % 10 == 1) {
				std::cout << ">> Recovery: Rotating to clear path... [" 
				          << recovery_rotation_frames << "/60] angle_diff=" 
